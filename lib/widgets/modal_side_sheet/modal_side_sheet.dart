...
Future<T?> showModalSideSheet<T extends Object?>({
  ...
  if (width == null) {
    if (deviceType == DeviceType.tablet) {
      // Giới hạn chiều rộng tối đa cho tablet là 600
      width = of.size.width * 0.6;
      if (width > 600) {
        width = 600;
      }
    } else if (platform == TargetPlatform.android || platform == TargetPlatform.iOS) {
      width = of.size.width * 0.6;
    } else {
  ...
  return showGeneralDialog(
    ...
    pageBuilder: (BuildContext context, _, __) {
      return Align(
        ...
        child: Material(
          elevation: elevation,
          child: SizedBox(width: width, height: height, child: withCloseControll ? Stack(children: [body, const Positioned(top: 5, right: 5, child: CloseButton())]) : body),
        ),
      );
    },
    transitionBuilder: (_, animation, __, child) {
      return SlideTransition(position: Tween<Offset>(begin: alignment == Alignment.bottomLeft ? const Offset(-1, 0) : const Offset(1, 0), end: Offset.zero).animate(animation), child: child);
    },
  );
}
