...
class TestTextNote extends StatefulWidget {
  const TestTextNote({super.key});

  @override
  State<TestTextNote> createState() => _TestTextNoteState();
}

class _TestTextNoteState extends State<TestTextNote> {
  ...
  void initState() {
    ...
    editorState = jsonString.then((value) {
      return EditorState(document: Document.fromJson(Map<String, Object>.from(json.decode(value))));
    });
  }
                  ...
                  children: [
                    FutureBuilder(
                      ...
                      builder: (context, snapshot) {
                        return !snapshot.hasData
                            ? const Center(child: CircularProgressIndicator())
                            : Expanded(child: SizedBox(child: AppFlowyEditor(editorStyle: EditorStyle.mobile(), shrinkWrap: true, editorState: snapshot.data!)));
                      },
                    ),
                    SizedBox(height: 36, child: _FixedToolbar(editorState: snapshot.data!)),
                  ],
                );
          },
        ),
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, selection, _) {
        return ListView.separated(
          ...
          itemBuilder: (_, index) {
            final isBold = _isTextDecorationActive(editorState, selection, AppFlowyRichTextKeys.bold);
            return IconButton(
              icon: Icon(items[index]),
              color: items[index] == Icons.format_bold && isBold ? Colors.blue : Colors.black,
              onPressed: () {
                ...
                switch (items[index]) {
                  ...
                  case Icons.check_box:
                    editorState.formatNode(null, (node) {
                      return node.copyWith(type: TodoListBlockKeys.type, attributes: {...node.attributes, TodoListBlockKeys.checked: false});
                    });
                    break;
                  ...
                  case Icons.format_list_bulleted:
                    editorState.formatNode(null, (node) {
                      return node.copyWith(type: node.type == BulletedListBlockKeys.type ? ParagraphBlockKeys.type : BulletedListBlockKeys.type);
                    });
                    break;
                  case Icons.format_list_numbered:
                    editorState.formatNode(null, (node) {
                      return node.copyWith(type: node.type == NumberedListBlockKeys.type ? ParagraphBlockKeys.type : NumberedListBlockKeys.type);
                    });
                    break;
                  case Icons.format_align_left:
                    editorState.formatNode(null, (node) {
                      return node.copyWith(attributes: {...node.attributes, blockComponentAlign: 'left'});
                    });
                    break;
                  case Icons.format_align_center:
                    editorState.formatNode(null, (node) {
                      return node.copyWith(attributes: {...node.attributes, blockComponentAlign: 'center'});
                    });
                    break;
                  case Icons.format_align_right:
                    editorState.formatNode(null, (node) {
                      return node.copyWith(attributes: {...node.attributes, blockComponentAlign: 'right'});
                    });
                    break;
